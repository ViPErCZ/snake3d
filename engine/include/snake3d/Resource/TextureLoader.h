...
namespace Resource {
    ...
    public:
        ...
        // pointSampled => NEAREST filtering + no mipmaps (palette atlases / pixel art).
        static unsigned int bindFromBuffer(const std::vector<unsigned char> &buffer, bool isAlbedo = true, bool pointSampled = false);
        static unsigned int bindFromBuffer(const void* buffer, unsigned int length, bool isAlbedo = true, bool pointSampled = false);
