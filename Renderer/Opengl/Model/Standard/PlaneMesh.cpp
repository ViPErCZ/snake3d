...
namespace Model {
    PlaneMesh::PlaneMesh(std::shared_ptr<BaseItem> baseItem, const float width, const float height) : StandardMesh(
        std::move(baseItem), width, height) {
        float hw = width * 0.5f;
        ...
        // Sestavení vrcholů (6 pro dva trojúhelníky)
        ...
        // Indexy (0 až 5, každý vertex jedinečný)
        indices.reserve(6);
        for (GLuint i = 0; i < 6; ++i) {
            indices.push_back(i);
        }

        // Příprava pro výpočet tangenty
        std::vector<glm::vec3> positions, normals;
        std::vector<glm::vec2> uvs; // ✅ správný typ
